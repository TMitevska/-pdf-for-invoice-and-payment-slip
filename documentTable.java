...
public class documentTable {
	...
	public void writeDataForTable(FontSelector fs) throws DocumentException{
	    
	    int rowNumber=getRowNumber();
	    for (int i=0;i<rowNumber;i++){
	    	tableData row=getRowData(fs);
	...
	private int getRowNumber(){
		System.out.println("Внеси број на редови во табела! пример(1,2,3,4...)");
		return Integer.parseInt(scanner.nextLine());
	}
	private tableData getRowData(FontSelector fs){
		System.out.println("Внеси име на артикал!");
		String data=scanner.nextLine();
		Phrase name=fs.process(data);
		System.out.println("Внеси име на мерка!");
		data=scanner.nextLine();
		Phrase merka=fs.process(data.toLowerCase());
		System.out.println("Внеси количина!");
		int kolicina=Integer.parseInt(scanner.nextLine());
		System.out.println("Внеси цена без ДДВ!");
		data=scanner.nextLine();
		int cena=Integer.parseInt(data);
